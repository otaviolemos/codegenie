...
public class TestDrivenSearchQuery implements ISearchQuery, Serializable {
	...
	public String getSourcererQuery() {
		...
		if (consideringNames) {
			...
			String terms = JavaTermExtractor.getFQNTermsAsString(querySpec[2]);
			
			if (consideringMissingClassName) 
				terms += " " + JavaTermExtractor.getFQNTermsAsString(querySpec[1]); 
			searchLabel += "Name terms: \'" + terms + "\' ";
			
			// TODO: Term or exact?
			
			query += " fqn_contents:(" + terms + ")";
		}
