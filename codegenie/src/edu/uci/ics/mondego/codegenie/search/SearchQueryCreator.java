...
//		  end = ((Integer) markers[myIndex].getAttribute(IMarker.CHAR_END)).intValue();
		...
		if (message.endsWith("cannot be resolved")) {
			wantedClassName = message.substring(0, message.indexOf("cannot be resolved") - 1);
			wantedMethodName = myCode.substring(end + 1, end + myCode.substring(end).indexOf('('));
			...
			myCode.substring(start + wantedClassName.length() + 1, 
					start + wantedClassName.length() + 1 + myCode.substring(start + wantedClassName.length() + 1).indexOf(" ="));
			int invocationStart = myCode.indexOf(instanceName+".") + instanceName.length() + 1;
		    wantedMethodName = myCode.substring(invocationStart,
		    		invocationStart + myCode.substring(myCode.indexOf(instanceName+".") + instanceName.length() + 1).indexOf('('));
			...
			wantedClassName = message.substring(message.indexOf("is undefined for the type") + 26);
		}
		...
		if (returnTypeNode != null) {
			...
			if (returnTypeNode.getNodeType() == ASTNode.VARIABLE_DECLARATION_FRAGMENT) {
				returnKEY = ((VariableDeclarationFragment)returnTypeNode).resolveBinding().getType().getKey();
				returnFQN = ((VariableDeclarationFragment)returnTypeNode).resolveBinding().getType().getQualifiedName();
				returnSN = ((VariableDeclarationFragment)returnTypeNode).resolveBinding().getType().getName();
			} else if (returnTypeNode.getNodeType() == ASTNode.METHOD_INVOCATION) {
				returnTypeExp = (Expression)((MethodInvocation) returnTypeNode).arguments().get(0);
				returnKEY = returnTypeExp.resolveTypeBinding().getKey();
				returnFQN = returnTypeExp.resolveTypeBinding().getQualifiedName();
		...
		keyInterface = wantedMethodName + myParamKeys + 
		returnKEY;
		...
		try {
			wantedPackageName = 
			  myTestType.getCompilationUnit().getPackageDeclarations()[0].getElementName();
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		String[] ret = {wantedPackageName, wantedClassName, wantedMethodName, returnSN, myParamSNs};
			...
			if (node.getName().getIdentifier().equals(wantedMethodName)) {
			  ...
			  if (node.getParent().getNodeType() == ASTNode.ASSIGNMENT) {
				  ...
				  return false;
					  ...
					  && ((MethodInvocation)node.getParent()).getName().getIdentifier().equals("assertEquals")) {
				  returnTypeNode = node.getParent(); //(Expression)((MethodInvocation) node).arguments().get(0);
				  return false;
			} else if (node.getParent().getNodeType() == ASTNode.VARIABLE_DECLARATION_FRAGMENT) {
			  returnTypeNode = node.getParent(); //((VariableDeclarationFragment) node.getParent()).resolveBinding();
			  return false;	
			}
			}
			return true;
